          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`font-medium text-xl transition-all duration-200 hover:scale-110 ${
                  scrolled
            ...
            <div className="max-w-7xl mx-auto px-4 py-4 space-y-3">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="block py-2 text-(--ngo-dark) hover:text-(--ngo-orange) font-medium"
                >
                  {link.label}
                </Link>
              ))}
              <div className="flex flex-col gap-3 pt-4 border-t">
                <Link
                  href="/get-involved#volunteer"
                  onClick={() => setIsOpen(false)}
                  className="flex items-center justify-center gap-2 px-5 py-3 rounded-full font-semibold border-2 border-(--ngo-green) text-(--ngo-green)"
                >
                  <Users className="w-4 h-4" />
                  Volunteer
                </Link>
                <Link
                  href="/get-involved#donate"
                  onClick={() => setIsOpen(false)}
                  className="flex items-center justify-center gap-2 px-5 py-3 rounded-full font-semibold bg-(--ngo-orange) text-white"
                >
                  <Heart className="w-4 h-4" />
                  Donate
                </Link>
